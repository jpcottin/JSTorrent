  ...
  chrome.runtime.sendMessage(appid, message, function (response) {
    if (response === undefined) {
      ...
      if (response.webapp) {
        ...
        showerror2(
          'JSTorrent window is not open or web server not enabled.  Check settings and reload',
        )
      }
    }
  })
...
function loadVideoUrl(baseUrl, d) {
  ...
  var shutdownErrorMessage =
    'JSTorrent App was closed. Playback will stop soon. You need to keep it open to play.'
  ...
  port.onMessage.addListener(function (msg) {
    ...
    if (msg.type == 'requestfileinfo') {
      ...
      sendport({ type: 'playerevent', event: 'initialized' })
  ...
  port.postMessage({ token: token, command: 'requestfileinfo', hash: d.hash, file: d.file })
}

function sendport(msg) {
  if (window.port) {
    window.port.postMessage(msg)
  }
}
...
function addevents(video) {
  ...
  function onevent(evt) {
    ...
    if (evt.type == 'progress') {
      if (state.lastrepeated > 40) {
        ...
        if (!state.sentplaying) {
          sendport({ type: 'playerevent', event: 'aplayworked' })
    ...
    if (evt.type == 'play') {
      sendport({ type: 'playerevent', event: 'clickplay' })
    } else if (evt.type == 'pause') {
      sendport({ type: 'playerevent', event: 'clickpause' })
    } else if (evt.type == 'seeked') {
      sendport({ type: 'playerevent', event: 'seeked' })
    }
    ...
    if (evt.type == 'error') {
      ...
      sendport({ type: 'playerevent', event: errtxt })
    } else {
