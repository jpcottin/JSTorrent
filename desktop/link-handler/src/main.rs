...
fn run(args: Args) -> Result<()> {
    ...
    let info =
        host_info.ok_or_else(|| anyhow::anyhow!("Failed to connect to JSTorrent Native Host"))?;
...
fn wait_for_host() -> Result<Option<ProfileEntry>> {
    ...
    Err(anyhow::anyhow!(
        "Timed out waiting for native host to start"
    ))
}
...
fn show_error(msg: &str, link: Option<&str>) {
    ...
    #[cfg(target_os = "windows")]
    {
        unsafe {
            let wide_msg: Vec<u16> = OsStr::new(&full_msg)
                ...
                .collect();
            let wide_title: Vec<u16> = OsStr::new("JSTorrent Error")
                .encode_wide()
                .chain(std::iter::once(0))
                .collect();
            MessageBoxW(
    ...
    {
        let script = format!(
            "display alert \"JSTorrent Error\" message \"{}\"",
            full_msg.replace('"', "\\\"")
        );
